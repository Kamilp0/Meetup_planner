...
$query = 'SELECT persona.nome, persona.cognome, persona.email, persona.dipartimento, persona.ruolo, invito.stato_notifica, invito.motivazione, invito.data_ultimo_aggiornamento 
            FROM persona 
            JOIN invito ON invito.persona=persona.email
            WHERE invito.id_riunione='.$id_riunione.';';
//echo $query;
...
while($row = mysqli_fetch_array($invitato)){
    ...
    switch($row['stato_notifica']){
        ...
        case 'confermato':
            ...
            $data_agg = '<small class="fst-italic text-secondary">  il '.$data_formattata.'</small>';
            break;
        case 'rifiutato':
            $colore = 'text-danger';
            $icona = '<i class="fas fa-calendar-times"></i>';
            $data_agg = '<small class="fst-italic text-secondary">  il '.$data_formattata.'<br>Motivazione: '.$row['motivazione'].'</small>';
            break;
        default:
            $colore = 'text-dark';
            $icona = '';
    ...
    echo '<tr><td>'.
        ...
        $row['cognome'].'</td><td>'.
        $row['email'].'</td><td>'.
        $row['dipartimento'].'</td><td>'.
        $row['ruolo'].'</td>
        <td class="'.$colore.'">'.$icona.'<strong>  '.$row['stato_notifica'].'</strong>'.$data_agg.'</td>
