...
$query = "SELECT nome, dipartimento, capienza, tavoli, proiettori, computer, lavagne FROM sala ";
...
while($row = mysqli_fetch_array($sale)){
    ...
    echo '<tr><td>'.
        $row['nome'].'</td><td class="tooltipkamil" messaggio-tooltip="'.$indirizzo.'">'.
        $row['dipartimento'].'</td><td>'.
        $row['capienza'].'</td><td>'.
        $row['tavoli'].'</td><td>'.
        $row['proiettori'].'</td><td>'.
        $row['computer'].'</td><td>'.
        $row['lavagne'].'</td>
        <td><a href="../frontend/modifica%20sala.php?sala='.$row['nome'].'&dip='.$row['dipartimento'].'" type="button" class="btn btn-primary btn-sm">Modifica</a></td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a type="submit" href="../backend/deletesala_back.php?sala='.$row['nome'].'&dip='.$row['dipartimento'].'" class="btn btn-danger">ELIMINA SALA</a>
                </div>
            </div>
        </div>
    </div>';
